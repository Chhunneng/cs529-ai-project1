...
2) Start core services (Postgres, Redis, backend, worker, frontend)
...
3) **Run DB migrations (required before `/api/v1/resume-templates` and related tables)**
...
- **Send message**: `POST /api/v1/sessions/{session_id}/messages` with JSON `{ "content": "..." }` → returns the **user** message row (includes `id`).
