...
async def try_claim_openai_conversation_id(*, session_id: uuid.UUID, conversation_id: str) -> bool:
    async with AsyncSessionMaker() as db:
...
async def ensure_openai_conversation_id(*, session_id: uuid.UUID, existing: str | None) -> str:
    if existing:
        return existing
    new_id = await create_openai_conversation()
    claimed = await try_claim_openai_conversation_id(session_id=session_id, conversation_id=new_id)
    if claimed:
        return new_id
...
async def handle_chat_message_job(job: ChatMessageJob) -> None:
    ...
    try:
        ...
        if intent.intent == "job_description":
            jd_id = await create_job_description_and_activate(
                session_id=session_id, raw_text=user_text
            )
            assistant_text = (
        ...
        assistant_id = await insert_assistant_message(
            ...
            tool_used=("internal.jd_ingest" if intent.intent == "job_description" else "openai.agents.Runner"),
        )
