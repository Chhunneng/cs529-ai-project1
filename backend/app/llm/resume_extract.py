...
_RESUME_EXTRACT_PROMPT = """Parse the resume plain text following these rules. Prioritize clean grouping: each job, school, or project should read as one unit; do not repeat the same fact on back-to-back rows.

Contact: label/value pairs only (Name, Location, Phone, Email, LinkedIn, GitHub, etc.). Values are plain text; keep URLs short without extra prose.

Summary: at most 3 sentences or ~120 words. If the resume has no summary, use "". Do not paste the entire Experience section into summary.
...
- For each employer/role block, use this pattern so readers see one coherent job:
  • depth 1: ONE line that already includes job title, company, location, and date range, e.g. "Software Engineer — Company Name Inc — USA — May 2023 – Jan 2026".
...
- If the source resume splits title and company on different lines, you may use depth 1 = company + dates + location and depth 2 = title, but never duplicate the same title on both rows.
...
def _user_message_for_extract(resume_text: str) -> str:
    return _RESUME_EXTRACT_PROMPT.format(resume_text=resume_text)
